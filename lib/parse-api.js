...
export function parseIIHFAPI(jsonData) {
  ...
  jsonData.forEach((game, index) => {
    ...
    // Parse date/time from GameDateTime (ISO local) and GameDateTimeUTC (UTC from API)
    ...
    // Format date as DD/MM/YYYY (IIHF local time for display)
    ...
    // Extract venue
    const venue = game.Venue || 'IIHF World Championship 2026';
