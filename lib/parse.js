...
/** First 3 letters of English month name → 1–12 */
function monthTokenToNum(token) {
  if (!token) return null;
  const key = String(token).toLowerCase().replace(/\./g, '').slice(0, 3);
  const map = { jan: 1, feb: 2, mar: 3, apr: 4, may: 5, jun: 6, jul: 7, aug: 8, sep: 9, oct: 10, nov: 11, dec: 12 };
  return map[key] ?? null;
}

/**
 * Parse a schedule date from free text (IIHF-style month + day, slashes, ISO).
 * @returns {string|null} DD/MM/YYYY or null
 */
function tryParseScheduleDate(text) {
  if (!text) return null;
  const monthWord =
    '(jan(?:uary)?|feb(?:ruary)?|mar(?:ch)?|apr(?:il)?|may|jun(?:e)?|jul(?:y)?|aug(?:ust)?|sep(?:t(?:ember)?)?|oct(?:ober)?|nov(?:ember)?|dec(?:ember))';
  let m = text.match(new RegExp(`\\b${monthWord}\\s+(\\d{1,2}),?\\s+2026`, 'i'));
  if (m) {
    const mo = monthTokenToNum(m[1]);
    const d = parseInt(m[2], 10);
    if (mo && d >= 1 && d <= 31) return `${d}/${mo}/2026`;
  }
  m = text.match(new RegExp(`\\b(\\d{1,2})\\s+${monthWord},?\\s+2026`, 'i'));
  if (m) {
    const d = parseInt(m[1], 10);
    const mo = monthTokenToNum(m[2]);
    if (mo && d >= 1 && d <= 31) return `${d}/${mo}/2026`;
  }
  m = text.match(/(\d{1,2})[\/\-](\d{1,2})[\/\-]2026/);
  if (m) {
    const a = parseInt(m[1], 10);
    const b = parseInt(m[2], 10);
    if (a >= 1 && a <= 31 && b >= 1 && b <= 12) return `${a}/${b}/2026`;
  }
  m = text.match(/2026[\/\-](\d{1,2})[\/\-](\d{1,2})/);
  if (m) {
    const mo = parseInt(m[1], 10);
    const d = parseInt(m[2], 10);
    if (mo >= 1 && mo <= 12 && d >= 1 && d <= 31) return `${d}/${mo}/2026`;
  }
  return null;
}

/**
 * Parse HTML from an IIHF schedule page and extract Team Canada games
...
export function parseSchedule(html, sourceUrl = '') {
  ...
  if (isIIHF) {
    ...
    $('table').each((tableIndex, table) => {
      ...
      const hasScheduleData = tableText.match(/\b(?:jan|january|feb|february|mar|march|apr|april|may|june|jul|july|aug|august|sep|september|oct|october|nov|november|dec|december)\s+\d{1,2}/i) ||
      ...
      $table.find('tr').each((rowIndex, row) => {
        ...
        const hasDate =
          rowText.match(
            /\b(?:jan|january|feb|february|mar|march|apr|april|may|june|jul|july|aug|august|sep|september|oct|october|nov|november|dec|december)\s+\d{1,2}/i
          ) || rowText.match(/\d{1,2}[\/\-]\d{1,2}[\/\-]2026/);
        ...
        if (hasCanada && (!hasDate || !hasTime)) {
          ...
          if ($prevRow.length > 0) {
            ...
            const monthDay =
              /\b(?:jan|january|feb|february|mar|march|apr|april|may|june|jul|july|aug|august|sep|september|oct|october|nov|november|dec|december)\s+\d{1,2}/i;
            if (prevText.match(monthDay) || prevText.match(/\d{1,2}[\/\-]\d{1,2}[\/\-]2026/)) {
          ...
          if ($nextRow.length > 0) {
            ...
            const monthDay =
              /\b(?:jan|january|feb|february|mar|march|apr|april|may|june|jul|july|aug|august|sep|september|oct|october|nov|november|dec|december)\s+\d{1,2}/i;
            if (nextText.match(monthDay) || nextText.match(/\d{1,2}[\/\-]\d{1,2}[\/\-]2026/)) {
  ...
  if (isIIHF) {
    ...
    $('[data-team], .team-code, [class*="team"], [data-country]').each((i, elem) => {
      ...
      if (parent.length > 0 && isCanadaGame(text)) {
        ...
        if (
          parentText.match(/\d{1,2}[\/\-]\d{1,2}[\/\-]2026/) ||
          parentText.match(
            /\b(?:jan|january|feb|february|mar|march|apr|april|may|june|jul|july|aug|august|sep|september|oct|october|nov|november|dec|december)\s+\d{1,2}/i
          ) ||
  ...
  if (games.length === 0) {
    ...
    $('body').find('*').each((i, elem) => {
      ...
      if (
        isCanadaGame(text) &&
        (/(?:jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)\b/i.test(textLower) ||
          textLower.includes('2026') ||
          text.match(/\d{1,2}[\/\-]\d{1,2}[\/\-]2026/))
      ) {
        const parsedDateStr = tryParseScheduleDate(text);
        const timeMatch = text.match(/(\d{1,2}):(\d{2})\s*(am|pm|AM|PM)?/);

        for (const pattern of gamePatterns) {
          const opponentMatch = text.match(pattern);
          if (opponentMatch && parsedDateStr && timeMatch) {
            let opponent = opponentMatch[1];

            // If no match[1], try to extract from the full match
            if (!opponent) {
              opponent = opponentMatch[0]
                .replace(/(?:canada|can\b)\s+vs\.?\s+/i, '')
            ...
            games.push({
              ...
              venue: extractVenue(text) || 'IIHF World Championship 2026',
              round: round,
              rawText: text.trim()
            });
  ...
  if (uniqueGames.length === 0 && isIIHF) {
    ...
    const dateMatches = bodyText.match(
      /\b(?:jan|january|feb|february|mar|march|apr|april|may|june|jul|july|aug|august|sep|september|oct|october|nov|november|dec|december)\s+\d{1,2}/gi
    );
...
function extractGameFromIIHFTableRow($row, cells, rowText, $) {
  ...
  // Extract date - look in cells first, then row text (IIHF month names, slashes, ISO)
  if (cells.length > 0) {
    cells.each((i, cell) => {
      if (dateStr) return false;
      const cellText = $(cell).text();
      const parsed = tryParseScheduleDate(cellText);
      if (parsed) {
        dateStr = parsed;
        return false;
      }
  ...
  if (!dateStr) {
    dateStr = tryParseScheduleDate(rowText);
  }
  ...
  // Extract venue
  venue = extractVenue(rowText) || 'IIHF World Championship 2026';
...
function extractGameFromStructuredData(data) {
  ...
  if (startDate && opponent) {
    ...
    return {
      ...
      timeStr,
      opponent,
      venue: location || 'IIHF World Championship 2026',
      round: 'Preliminary',
      rawText: name
    };
  }
