...
/**
 * Convert local Switzerland summer time (CEST, UTC+2) to UTC — used for HTML-scraped WM dates/times.
...
export function parseDateTime(dateStr, timeStr) {
  ...
  if (dateParts.length === 3) {
    ...
    // If both parts <= 12, prefer DD/MM/YYYY for European / IIHF sources
    ...
    if (part1 > 12) {
      ...
      // IIHF / European sources: assume DD/MM/YYYY
      day = part1;
      month = part2 - 1;
      year = part3;
    }
  ...
  // WM 2026 (May): Switzerland is on CEST (UTC+2). Scraped times are treated as local arena time.
  const localOffsetHours = 2;
  ...
  const utcDate = new Date(localDate.getTime() - (localOffsetHours * 60 * 60 * 1000));
...
export function generateICS(games, sourceUrl) {
  const calendar = ical({
    ...
    // Don't set timezone here - events will use UTC explicitly
    // This ensures DTSTART/DTEND have 'Z' suffix for UTC time
    url: sourceUrl,
    source: sourceUrl
    // TTL removed - not available in this version of ical-generator
  });

  // Add calendar metadata (must start with X-)
  ...
  games.forEach((game, index) => {
    try {
      ...
      // If game has utcDate (from API), use it directly for accurate timezone handling
      let startDate;
      if (game.utcDate && game.utcDate instanceof Date && !isNaN(game.utcDate.getTime())) {
        // Use the UTC date from API - this is already correctly converted
        startDate = game.utcDate;
        console.log(`    Using UTC date from API: ${startDate.toISOString()}`);
      } else {
        // Fallback to parsing date/time strings (for HTML-scraped games)
        startDate = parseDateTime(game.dateStr, game.timeStr);
        console.log(`    Parsed date from strings: ${startDate.toISOString()}`);
      }
      ...
      const summary = `Canada vs ${normalizedOpponent} (Men's Worlds 2026)`;
